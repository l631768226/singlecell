...
public interface UmapDataMapper {
    ...
    @Select("select celltype from umapdata where dataset = #{dataset} group by celltype ORDER BY celltype asc")
    List<String> findCelltypeListByDataset(@Param("dataset")String dataset);
